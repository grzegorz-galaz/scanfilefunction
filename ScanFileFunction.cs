...
public class ScanFileFunction
{
    ...
    public ScanFileFunction(ILogger<ScanFileFunction> logger, ClamAvScanner scanner, EventPublisher publisher, BlobServiceClient blobServiceClient)
    {
        _logger = logger;
        _scanner = scanner;
    ...
    {
        // Krok 1: Pobieramy dane z eventu (czyli URL pliku)
        ...
        // Krok 2: Pobieramy plik z Blob Storage
        var containerClient = _blobServiceClient.GetBlobContainerClient(containerName);
        var blobClient = containerClient.GetBlobClient(fileName);
        var stream = await blobClient.OpenReadAsync();

        // Krok 3: Skanujemy przez ClamAV
        var result = await _scanner.ScanAsync(stream);

        // Krok 4: Emitujemy customowe zdarzenie
        ...
        _logger.LogInformation("Scan finished: {Status} - File: {FileName}", result.IsInfected ? "INFECTED" : "CLEAN", fileName);
    }
}
